...
import { updateVenueByIdService } from '../../../Services/venueServices';

const FormEditVenues = ({dt_object}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await updateVenueByIdService(token,formObject,element_id);
            if (response.status === 200 || response.status === 201) {
                ...
                console.log('Venue edit successful', data);
            }
            else{
                ...
                throw new Error('Failed to submit Venue edit');
            }
        } catch (err) {
            e.target.parentNode.querySelector('.alert-danger').classList.remove('hidden');
            e.target.parentNode.querySelector('.alert-success').classList.add('hidden');
  ...
  return (
    <div>
        <form id="editVenuesForm"  onSubmit={handleSubmit}>
            <div className="row">
                <div className="col-6">
        ...
        <div className="alert alert-success hidden" role="alert">
            Locacion editada correctamente.
        </div>
        <div className="alert alert-danger hidden" role="alert">
            Hubo un error al editar la locacion.
        </div>
    </div>
  )
}

export default FormEditVenues